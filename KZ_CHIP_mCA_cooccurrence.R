...
for (Gene.refGene in gene_chip) {
  for (chr_type3 in colnames(contingency_table)) {
    sub_table <- matrix(c(
      contingency_table[Gene.refGene, chr_type3],           
      sum(contingency_table[Gene.refGene,]) - contingency_table[Gene.refGene, chr_type3],  
      sum(contingency_table[, chr_type3]) - contingency_table[Gene.refGene, chr_type3], 
...
final_results <- do.call(rbind, lapply(names(or_results), function(name) {
  data.frame(
    ...
    co_occurrence = or_results[[name]]$co_occurrence  
  )
}))
